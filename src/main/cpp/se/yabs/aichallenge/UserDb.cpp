...
const mgen::Field * UserDb::_fieldByName(const std::string& name) const {
	static const std::map<std::string, const mgen::Field*> name2meta = mgen::make_map<std::string, const mgen::Field*>()("users", &UserDb::_field_users_metadata());
...
const std::vector<mgen::Field>& UserDb::_field_metadatas() {
	static const std::vector<mgen::Field> out = mgen::make_vector<mgen::Field>() << _field_users_metadata();
	return out;
}
